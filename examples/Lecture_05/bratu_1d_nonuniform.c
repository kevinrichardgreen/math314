...
The command line options include:\n\
  -par <parameter>, where <parameter> indicates the problem's nonlinearity\n\
       <parameter> = Bratu parameter (0 <= par <= 3.51)\n\n";
/* ------------------------------------------------------------------------
...
PetscScalar CoordinateTransformFunction(PetscScalar x)
{
  ...
  /* return PetscPowScalar(x,1.0/3.0); */
}
#undef __FUNCT__
  ...
  */
  ...
  for (i=xs; i<xs+xm; i++) {
    if (i == 0 || i == Mx-1) {
      ...
      x[i] = PetscSinReal(PETSC_PI*i*coors[i]);
    }
  }
  /* Restore vector */
...
PetscErrorCode FormFunctionLocal(DMDALocalInfo *info,PetscScalar *x,PetscScalar *f,AppCtx *user)
{
  ...
  for (i=info->xs; i<info->xs+info->xm; i++) {
    if (i == 0 || i == info->mx-1) {
      ...
      uxx  = 2.0*((he+hw)*u - he*uw - hw*ue)/(he*hw*hw + hw*he*he);

      /* evaluate the residual */
      f[i] = uxx - lambda*PetscExpScalar(u);
    }
  }
...
PetscErrorCode FormJacobianLocal(DMDALocalInfo *info,PetscScalar *x,Mat jac,Mat jacpre,AppCtx *user)
{
  ...
  PetscScalar    lambda,v[3],he,hw,factor;
  DM             coordDA;
  Vec            coordinates;
  PetscScalar   *coors;
  ...
  */
  for (i=info->xs; i<info->xs+info->xm; i++) {
    ...
    hw = coors[i] - coors[i-1];
    he = coors[i+1] - coors[i];
    factor = 2.0/(he*hw*hw+hw*he*he);

    row.i = i;
    /* boundary points */
    if (i == 0 || i == info->mx-1) {
      v[0] =  2.0; /* NOTE: either hw or he are undefined at BC */
	...
	k++;
      ...
      /* current grid point */
      v[k] = factor*(he+hw) - lambda*PetscExpScalar(x[i]);
      col[k].i = row.i;
      k++;
      /* east point */
      if (i+1 != info->mx-1) {
